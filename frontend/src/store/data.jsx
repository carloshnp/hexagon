...
/* ==================== Time-window presets — 3-month rolling weeks ==================== */
...
function generateTimeWindows() {
  ...
  const wins   = [{ key: '84d', label: 'Últimos 3 meses', tw: '84d' }];
  for (let w = 1; w <= 13; w++) {
    const days      = w * 7;
    const endDate   = new Date(now - (w - 1) * 7 * MS_DAY);
    const startDate = new Date(now - days * MS_DAY);
    const tag   = w === 1 ? ' (atual)' : '';
    const label = `Sem. ${String(w).padStart(2,'0')} · ${_fmtDate(startDate)}–${_fmtDate(endDate)}${tag}`;
    wins.push({ key: `w${String(w).padStart(2,'0')}`, label, tw: `${days}d` });
  }
  return wins;
}

const TIME_WINDOWS  = generateTimeWindows();
const DEFAULT_PERIOD = '84d';

function twForPeriod(periodKey) {
  return TIME_WINDOWS.find(w => w.key === periodKey)?.tw || '84d';
}
...
const dataStore = {
  ...
  async loadReport(fid) {
    ...
    try {
      const [regionReport, regionDetail] = await Promise.all([
        apiFetch(`/reports/regions/${regionId}?time_window=${tw}`),
        apiFetch(`/map/regions/${regionId}?time_window=${tw}`).catch(() => ({})),
      ]);
